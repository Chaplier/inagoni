...
real32 TNoiseBase::RidgedMultifractal( const TVector3& point, const real32 samplingRate)
{
	const real32 cutoff = Clamp(2,fMaxFrequency,.5/samplingRate);
	const real32 freqMax=.5*cutoff;
	...
	for( f=kMinFrequency; f<freqMax ; f*=kFractalIncrement )
	{
		real32 signal = (1 - RealAbs( GetValueSmooth(point*f) ));
		signal *= signal; // to increase sharpness of the ridges
		signal *= weight;
		signal *= RealPow(f, exp);///f; // p505, didn't divide by f, but result ofen greater than 1, so I do it
		
		value += signal;

		// Compute the next weight
		weight = MC_Clamp(signal*gain, kRealZero, kRealOne);
	}
	real32 fade=Clamp(0,1,2*(cutoff-f)/cutoff);
	{
		real32 signal = (1 - RealAbs( GetValueSmooth(point*f) ));
...
void TNoiseBase::InitGradientTab()
{
	...
	int32 iPerm;
	for(iPerm=0 ; iPerm<kTabSize ; iPerm++)
	{
	...
	int32 index;
	for(index=0 ; index<kTabSize ; index++)
	{
