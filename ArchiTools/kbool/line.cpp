...
KBoolLine::KBoolLine( KBoolLink *a_link, Bool_Engine* GC )
{
    ...
    // must be an if statement because if an MY_ASSERT is used there will
...
//
void KBoolLine::AddLineCrossing( B_INT X, B_INT Y, double Elevation, KBoolLine *other_line ) //OKTAL MODIFICATION
{
    // the other line must exist
    MY_ASSERT( other_line );
    // the links of the lines must exist
    MY_ASSERT( other_line->m_link && m_link );
	other_line->AddCrossing( AddCrossing( X, Y , Elevation) ); //OKTAL MODIFICATION
}
...
void KBoolLine::CalculateLineParameters()
{
    ...
    if ( !m_valid_parameters )
    {
		...
		myElevation = bp->GetElevation(); //OKTAL MODIFICATION
    ...
    switch ( Take_Action1 )
    {
        ...
        case 1:
        {
            ...
            switch ( Take_Action2 )
            {
                ...
                default: Total_Result = false; MY_ASSERT( Total_Result );
            }
        }
...
case 1: case 5: case 6:
        {
            ...
            switch ( Take_Action2 )
            {
                ...
                {   // begin of scope to calculate the intersection
					...
					double Elevation = myElevation; //OKTAL MODIFICATION

                    //make a decent rounding to B_INT
					AddLineCrossing( ( B_INT )X, ( B_INT )Y,  Elevation ,lijn ); //OKTAL MODIFICATION : add Elevation
...
int KBoolLine::Intersect_simple( KBoolLine * lijn )
{
	...
	AddLineCrossing( ( B_INT )X, ( B_INT )Y, myElevation,lijn ); //OKTAL MODIFICATION : add Elevation
...
void KBoolLine::Set( KBoolLink *a_link )
{
    ...
    // must be an if statement because if an MY_ASSERT is used there will
    // be a macro expansion error
// if (a_link->GetBeginNode()->Equal(a_link->GetEndNode(),MARGE)) MY_ASSERT(!a_link);
...
{
    m_GC = a_line.m_GC;
	myElevation = a_line.myElevation;
    m_AA = a_line.m_AA;
    m_BB = a_line.m_BB;
    m_CC = a_line.m_CC;
...
bool KBoolLine::OkeForContour( KBoolLine* const nextline, double factor, Node* LastLeft, Node* LastRight, LinkStatus& _outproduct )
{
    MY_ASSERT( m_link );
    MY_ASSERT( m_valid_parameters );
    MY_ASSERT( nextline->m_link );
    MY_ASSERT( nextline->m_valid_parameters );
...
// see above
//
Node* KBoolLine::AddCrossing( B_INT X, B_INT Y, double Elevation ) //OKTAL MODIFICATION
{
	Node * result = new Node( X, Y, Elevation,m_GC );
    AddCrossing( result );
    return result;
}
