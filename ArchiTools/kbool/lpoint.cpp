...
LPoint::LPoint()
{
    _x = 0;
    _y = 0;
	myElevation = 0; //OKTAL MODIFICATION
}


LPoint::LPoint( B_INT const X, B_INT const Y, double const Elevation ) //OKTAL MODIFICATION : add elevation
{
    _x = X;
    _y = Y;
	myElevation = Elevation; //OKTAL MODIFICATION
}


LPoint::LPoint( LPoint* const a_point ) // OKTAL MODIFICATION : Add elevation
{
    if ( !a_point )
    ...
    _x = a_point->_x;
    _y = a_point->_y;
	myElevation = a_point->myElevation; //OKTAL MODIFICATION
}
...
double LPoint::GetElevation()//OKTAL MODIFICATION
{
    return myElevation;
}
...
{
	myElevation = other_point.myElevation;
    _x = other_point._x;
    _y = other_point._y;
    return *this;
}
