...
bool ScanBeam::ProcessHoles( bool atinsert, TDLI<KBoolLink>* _LI )
{
    ...
    if ( !record->GetLine()->CrossListEmpty() )
    {
        ...
        while( !I.hitroot() )
        {
			...
			Node *node_A = new Node( topnode->GetX(), Y, leftnode->GetElevation(),_GC );  //OKTAL MODIFICATION 
    ...
    if ( link->IsTopHole() && !_BI.athead() )
    {
        ...
        MY_ASSERT( record->Direction() == GO_LEFT );
        // he goes to the left
        if ( atinsert )
        {
...
int ScanBeam::Process_LinkToLink_Flat( KBoolLine* flatline )
{
    ...
    for( _BI.tohead(); !_BI.hitroot(); _BI++ )
    {
        ...
        { //it is in between the flat link region
            ...
            {
				Node * newnode = new Node( _low->GetX(), _BI.item()->Ysp(), _low->GetElevation(),_GC ); //OKTAL MODIFICATION
