...
// Default constructor
//
KBoolLink::KBoolLink( Bool_Engine* GC ) : m_Inc(false), m_graphnum(0),m_record(0)
{
    _GC = GC;
    Reset();
}
...
{
    MY_ASSERT( node == m_beginnode || node == m_endnode );
...
// MY_ASSERT ((begin_or_end_node == beginnode)||(begin_or_end_node == endnode));
...
LinkStatus KBoolLink::OutProduct( KBoolLink* const two, double accur )
{
    ...
    {
        center = m_beginnode;
//  MY_ASSERT(center==two->endnode || center==two->beginnode);
    }
...
//  MY_ASSERT(center==two->endnode || center==two->beginnode);
//m }
// MY_ASSERT(center==third->endnode || center==third->beginnode);
    ...
    switch( TwoToOne )
    {
        ...
        default: Result = IS_ON; MY_ASSERT( false );
    }
    return Result;
}
