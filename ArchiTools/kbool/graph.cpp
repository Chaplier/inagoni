...
Graph::Graph( Graph* other )
{
	...
	Node* node = new Node( _current->GetBeginNode()->GetX(), _current->GetBeginNode()->GetY(), _current->GetBeginNode()->GetElevation(),_GC ); //OKTAL MODIFICATION
    Node* nodefirst = node;
    for ( int i = 0; i < _nr_of_points; i++ )
    {
        ...
        Node* node2 = new Node( _current->GetBeginNode()->GetX(), _current->GetBeginNode()->GetY(), _current->GetBeginNode()->GetElevation(),_GC );
...
void Graph::CreateArc( Node* center, Node* begin, Node* end, double radius, bool clock, double aber )
{
    ...
    for ( i = 1; i < Segments; i++ )
    {
        ...
        _current = new Node( ( B_INT ) ( center->GetX() + radius * cos( phi - dphi ) ),
							...
							center->GetElevation(), _GC );//OKTAL MODIFICATION
...
void Graph::Make_Rounded_Shape( KBoolLink* a_link, double factor )
{
    ...
    for ( i = 1; i < Segments; i++ )
    {
        ...
        _current = new Node( ( B_INT ) ( a_link->GetEndNode()->GetX() + factor * cos( phi - dphi ) ),
                             ( B_INT ) ( a_link->GetEndNode()->GetY() + factor * sin( phi - dphi ) ), a_link->GetEndNode()->GetElevation() ,_GC ); //OKTAL MODIFICATION
    ...
    for ( i = 1; i < Segments; i++ )
    {
        ...
        _current = new Node( ( B_INT )( a_link->GetBeginNode()->GetX() + factor * cos( phi - dphi ) ),
			( B_INT )( a_link->GetBeginNode()->GetY() + factor * sin( phi - dphi ) ), a_link->GetBeginNode()->GetElevation(), _GC ); //OKTAL MODIFICATION
