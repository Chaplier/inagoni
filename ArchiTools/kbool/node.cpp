...
Node::Node( Bool_Engine* GC ) : LPoint( 0, 0 , 0)  //OKTAL MODIFICATION : add elevation
{
    _GC = GC;
...
Node::Node( B_INT const X, B_INT const Y, double const Elevation, Bool_Engine* GC ) : LPoint( X, Y, Elevation) //OKTAL MODIFICATION : add elevation
{
    _GC = GC;
    _linklist = new DL_List<void*>();
}
...
Node::Node( LPoint* const a_point, Bool_Engine* GC ) : LPoint( a_point ) //OKTAL MODIFICATION : add elevation
{
    _GC = GC;
    _linklist = new DL_List<void*>();
}
...
Node::Node( Node * const other, Bool_Engine* GC )
{
    ...
    _x = other->_x;
    _y = other->_y;
	myElevation = other->myElevation;//OKTAL MODIFICATION
...
Node& Node::operator=( const Node &other_node )
{
    ...
    _x = other_node._x;
    _y = other_node._y;
	myElevation = other_node.myElevation; //OKTAL MODIFICATION
    return *this;
}
...
bool Node::Simplify( Node *First, Node *Second, B_INT Marge )
{
    ...
    /*if ( tmp_line.PointOnLine( this, distance, ( double ) Marge ) == ON_AREA )  //OKTAL MODIFICATION 
    {
